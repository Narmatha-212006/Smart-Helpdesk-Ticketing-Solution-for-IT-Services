...
import type { Role, BackendUser } from "@/types";
...
const toManagedUser = (backendUser: BackendUser, index: number): ManagedUser => ({
  ...
  createdAt: new Date().toISOString().split("T")[0], // Backend doesn't return, use current
  avatar: backendUser.name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2),
});
...
// ── Empty form ────────────────────────────────────────────────
const emptyForm = { name: "", email: "", password: "", role: "user" as Role, department: "", status: "Active" };
...
export default function UsersPage() {
  ...
  if (!currentUser || currentUser.role !== 'admin') {
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-muted-foreground">Admin role required for user management.</p>
      </div>
    );
  }
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      setError(null);
      const data: BackendUser[] = await apiFetch('/api/users');
      const managedUsers = data.map((u, i) => toManagedUser(u, i));
    ...
    try {
      if (editTarget) {
        ...
        toast({
          ...
          description: "Full CRUD coming soon",
        });
      } else {
        ...
        toast({
          ...
          description: `${form.name} has been added with ${form.role} role.`,
        });
        fetchUsers();
      }
      setModalOpen(false);
