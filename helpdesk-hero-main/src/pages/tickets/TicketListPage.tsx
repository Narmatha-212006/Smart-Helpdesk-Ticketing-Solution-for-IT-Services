...
export default function TicketListPage({ myTickets = false }: Props) {
    ...
    if (myTickets) {
      const loadMyTickets = async () => {
        try {
          ...
          // Map backend DTO to frontend (normalize status/priority)
          const mapped = data.map((t: any): Ticket => ({
            id: t.id.toString(),
            title: t.title,
            description: t.description,
            category: t.category as TicketCategory,
            priority: t.priority.charAt(0).toUpperCase() + t.priority.slice(1).toLowerCase() as TicketPriority,
            status: t.status === "OPEN" ? "Open" : t.status === "IN_PROGRESS" ? "In Progress" : "Closed" as TicketStatus,
            createdById: t.createdBy.id.toString(),
            createdByName: t.createdBy.name,
            assignedToId: t.assignedTo ? t.assignedTo.id.toString() : undefined,
            assignedToName: t.assignedTo ? t.assignedTo.name : undefined,
            attachments: [],
            comments: [],
      ...
      // For all tickets, keep existing logic if context available, but for now mock empty for non-my
      setTickets([]);
      setLoading(false);
    }
  }, [myTickets]);
  ...
  const handleDelete = async () => {
    if (deleteTarget) {
      try {
        // Assume backend DELETE /api/tickets/{id} - call it
        await fetch(`/api/tickets/${deleteTarget.id}`, { method: 'DELETE' });
  ...
  if (loading) {
    return (
      <div className="p-6 text-center text-muted-foreground">
        Loading {myTickets ? 'my tickets' : 'tickets'}...
      </div>
    );
  }
