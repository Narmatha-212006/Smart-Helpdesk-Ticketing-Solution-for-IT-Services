...
export default function CreateTicketPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Failed to create ticket", description: err.message || "Please try again.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-center min-h-[60vh] p-6 animate-fade-in">
        <div className="bg-card rounded-2xl border border-border shadow-card p-10 text-center max-w-md w-full">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link
              to="/dashboard"
              className="px-5 py-2.5 text-sm bg-secondary hover:bg-secondary-dark text-secondary-foreground rounded-lg transition-colors font-medium"
            >
              Go to Dashboard
            </Link>
          </div>
